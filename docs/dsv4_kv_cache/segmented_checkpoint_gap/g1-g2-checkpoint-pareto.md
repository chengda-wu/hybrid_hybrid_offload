...
代表点上没有同-gap 方案更优。两个零差值点本来就已经满足 $g_1=g_2$；其余七点强制使用同一个 gap 后，精确恢复量增加 0.710% 到 14.246%。尤其是 4 GiB 和 8 GiB 点，gap 数值只差 3 和 1，但收益分别达到约 14.25% 和 7.18%，说明这不是可以忽略的数值扰动。

原因是同 gap 会把两段相位锁定为 $r_2=(r_1-ld)_+$；即使只把 $g_2$ 改动 1，$b_1\bmod g_2$ 也会解除这种强耦合。相邻且互质的 gap 可以显著改变第二段的相位分布，所以“gap 很接近”并不意味着恢复行为也很接近。

### 8.3 对现有前沿完整性的补充发现

完整同-gap 扫描是原约束空间的子集，理论上不可能优于真正完整的双-gap 前沿。但与当前三级启发式搜索输出的 21213 个前沿点逐点比较时，有 213 个存储位置出现同-gap 采样恢复量更低，最小相对差值为 -4.073%；把同-gap 点并入后，合并前沿包含 214 个同-gap 点。这说明原搜索在部分细粒度区间漏掉了 $g_1=g_2$ 这条轴上的候选，并不表示同-gap 能超越完整的双-gap 最优解。

新增交互图在原图下方绘制“同一存储上限下，强制同 gap 的恢复量相对差异”：正值表示同 gap 更差，负值表示发现了当前启发式前沿的遗漏。代表预算点使用圆点标出。
