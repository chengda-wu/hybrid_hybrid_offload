...
> **与 [swa-kv-offloading-analysis.md](swa-kv-offloading-analysis.md) 的关系**：那份文档给出 SWA 依赖锥的一般形式化（$R_\ell$ 递推、DRAM/HBM/计算三轴），用 DSV4 Pro（$L=61$）估算；本文落到 DSV4 Flash（$L=43$）的 APC delta 段。本文三角形即该文档 §3 的依赖锥（§11.4 等价），方向 A 的浅层边界带即该文档 §4 的 $|R_m|$，方向 D 的 token 维周期存储即该文档 §7 的 token checkpoint。
...
计算节省来自 skip+staircase，与是否松绑无关。token 维周期存储（方向 D）是另一正交变体，进一步降到 $M\cdot L$，见 §15.6。
...
§11–§13 的 staircase 把 43 层一视同仁。本节沿多个正交方向探索"更优"的可能，不预设单一方案：

- **层维**：保留部分层（方向 A，§15.3）、层维 checkpoint（方向 B，§15.4）、C4/C128 层间差异化（方向 C，§15.5）
- **token 维**：周期存储——每隔 $M$ token 存一次 SWA 边界 checkpoint（方向 D，§15.6）

> **术语澄清**："周期存储"指 **token 维周期**（每 $M$ token 一个 SWA checkpoint，对应 `swa-kv-offloading-analysis.md` §7），**不是** §15.1 的层间 C4/C128 交替。两者正交，分属不同方向。

### 15.1 DSV4 层间写回成本差异（C4/C128 交替）
...
- **C4 与 C128 严格交替**（层间周期 = 2 层）

这是模型架构自带的**层间压缩比交替**，决定了各层写回 main MLA 压缩条目的成本天差地别：

| 层类型 | 层数 | 每 token 条目 | D=5500 delta 总条目 |
|---|---|---|---|
| C4 (cr=4) | 21 | 1/4 | **28 875** |
| C128 (cr=128) | 20 | 1/128 | **~859** |
| SWA-only | 2 | 0 | 0 |

→ **C128 层写回成本比 C4 层低约 34×**（28875 / 859 ≈ 33.6）。这是方向 C（§15.5）的物理基础。注意此处的"周期"是**层间交替**，与 §15.6 的 token 维周期存储是不同概念。
...
保留 K 层完整 cache、其余 43−K 层做三角形。先看朴素选法——保留**浅层**前 K 层：分界层 $K$ 处深层三角形的底宽为 $|R_K| \approx W+(43-K)(W-1)$（top-W 口径，与 §11.4 一致；`swa-kv-offloading-analysis.md` §4 用 top-1 口径得 $1+(L-m)(W-1)$，差一个 $W-1$）。

注意：保留的是 **KV**，不是 hidden state。浅层 KV 存了，但高层恢复需要浅层的 **hidden state**（高层 attention 输入是下层 hidden），hidden 未存 → 须把 $|R_K|$ 个位置推过浅层 K 层重算 hidden。故浅层保留成本 = $K\cdot|R_K|$（重算浅层 hidden 喂高层）+ 高层缺失段锥。K=21 时 $\approx 61k+32k=93k$，**低于**全三角形（117k）——浅层保留仍省于不保留。§15.3.1 证明保留**深层**恢复计算更低。
...
> **与 DSV4 层间结构契合**：C128 层（cr=128）写回极低（~859 条，§15.1）且与 C4 交替遍布浅中深，故"存深层"在 DSV4 上部分天然免费——C128 近乎免费保持完整，与"存深层更优"同向。写回主体是 C4 层（28875 条），其浅/深差异已含上表。若叠加"保留部分层 cache"，**深层保留 + C4 staircase 兜底浅层**是计算与复用兼优的组合（与方向 C 同向，§15.5）。

### 15.4 方向 B：层维 checkpoint（❌ 不推荐）

每 $S$ 层存一次完整 cache、中间层不存（对应 `swa-kv-offloading-analysis.md` §6 的层维 checkpoint）。层间连续性要求 checkpoint 间连续 forward 重算，等同激活重计算——**不省计算，只省 cache 内存**，对三角形收益无帮助。（区别于 §15.6 的 token 维 checkpoint——后者按 token 间距存、可截断依赖锥。）
...
**层间连续性的硬约束**：不能逐层独立做。C128 层全量算，需要相邻 C4 层的全部 delta token 隐藏态作输入；C4 层若独立做三角形砍了左缘 → **矛盾**（§15.2）。故方向 C 必须按 **(C4, C128) 周期对**整体处理：每两层一个周期，周期内 C4+C128 一起算同样多 token，周期之间三角形收敛。

```
方向 C：按周期对 (C4, C128) 一起做三角形
...
方向 C 的价值不在计算，而在 main MLA 压缩条目的写回成本：

- **C4 层是写回主体**：`D/4·21 = 28875` 条，三角形 + skip-writeback 省下。
- **C128 层写回可忽略**：cr=128，全 delta 写回也只 `D/128·20 ≈ 859` 条（D=5500）→ 即便 C128 层不做三角形、全量写回也几乎免费，保持 C128 main MLA 条目完整。
- C4 层 skip-writeback 独立于三角形也成立——这才是 DSV4 层间结构带来、可独立落地的收益。

> **"破坏完整性"指哪类 cache**：不是 main MLA——它的压缩条目是**全序列**保存的（G0 命中到 A，`[0,A)` 都在，§10.3），delta 段 `[B,A)` 的 main MLA 条目本就完整、本就 skip 写回。被破坏的是 **SWA ring / compressor state 这类滑窗 cache**：它们只存末尾 window，左缘 delta token `[B, B+D-W_eff)` 的 SWA KV 既不在 ring（被覆盖）也不写回 → delta 段留下三角形空洞（缺位，非值错）。当前 decode 无碍（右缘 window 已缓存）；但后续请求若共享前缀 `[B,A)`，APC 按 block hash 查 SWA/compressor block 会因左缘缺失而命中变短 → 那段须重算。即 staircase 用"放弃 delta 前缀 SWA/compressor 的未来复用"换当前省计算。

| 方案 | 计算省量 | main MLA 写回 | SWA/compressor 滑窗 cache |
|---|---|---|---|
| 原全三角形（§11） | ~50% | 全 skip（条目本就完整） | ❌ 留三角形空洞 |
| 方向 C（周期对 + C4 skip） | ~54% | C4 skip、C128 全量写回（≈免费） | C4 留洞、C128 可全量保留 |

方向 C = 同等计算省量 + C128 滑窗 cache 也能保完整（写回近乎免费）。

### 15.6 方向 D：Token 维周期存储（SWA checkpoint）

与前述层维方向不同，本方向在 **token 维**做周期存储：每隔 $M$ token 存一次 SWA 边界 checkpoint（对应 `swa-kv-offloading-analysis.md` §7）。在 checkpoint 位置 $c$ 保存每层窗口末端的 full KV 作为可续 SWA 的边界状态：

$$
\mathcal{B}_c = \{\, KV_t^{(\ell)} \mid \ell=1,\dots,L,\ t\in\{c-W+1,\dots,c\} \,\}
$$

checkpoint 集合 $\mathcal{C}=\{M,2M,\dots\}$，间距 $M$（示例 $M=1024$）。恢复 delta 段尾部 $A$ 时取最近 $c=\max\{x\in\mathcal{C}\mid x\le A\}$，从 $\mathcal{B}_c$ 顺序重算 $[c+1,A]$，恢复长度 $r=A-c<M$。

**计算节省——截断依赖锥**：staircase 是三角形 $W_{eff}\cdot L/2$，因为位置 $A$ 的尾部 window 要从 layer 0 回溯整条锥（底宽 $W_{eff}$）。checkpoint 在 $c$ 提供**每层边界 KV**，恢复 $[c+1,A]$ 时每层有起点窗口，**不再向低层回溯**——退化为横向推 $r$ 个 token 过所有层的矩形 $L\cdot r\le M\cdot L$。

| 方案 | delta 段恢复计算 | $D{=}5500$ 时 | vs 矩形 $D\cdot L$ |
|---|---|---|---|
| 当前全量重算 | $D\cdot L$ | $236k$ | 基线 |
| staircase（§11） | $\approx W_{eff}\cdot L/2$（$D\ge W_{eff}$） | $118k$ | 省 ~50% |
| 方向 D（token checkpoint） | $\le M\cdot L$（$D\ge M$） | $44k$（$M{=}1024$） | 省 ~81% |
| D + staircase | $\le M\cdot L/2$（$D\ge M$） | $22k$ | 省 ~91% |

（$L=43$，$W_{eff}=5462$，$M=1024$。）

- **生效门槛**：staircase 需 $D\ge W_{eff}\approx 5500$；方向 D 只需 $D\ge M=1024$。$M\ll W_{eff}$，D 更早生效——$D\in[1024,5500)$ 时 staircase 尚未展开（$\approx D\cdot L$），D 已固定 $\le M\cdot L$。
- **根因**：staircase 截断锥的左缘（靠前 delta token 深层），仍向低层回溯；D 截断锥的高度（向低层回溯本身）。两者正交，叠加得 $M\cdot L/2$。
- **$D<M$ 时退化**：段内无 checkpoint（$r\ge D$），D 回退 $D\cdot L$，此时 staircase 更优。两方案互补：$D$ 大用 D，$D$ 小用 staircase。

> **前提**：① checkpoint 独立常驻，不随 SWA ring 滑窗 evict；② full-state（每层窗口 full KV，非单点——单点无法续 SWA）；③ **非 vLLM 当前实现**，为设计变体；④ delta 段 [B,A) 的 SWA cache 不在（命中到 B），段内 checkpoint 须独立常驻且 $D\ge M$ 才有 checkpoint 落在段内。
>
> **存储权衡**：常驻 $\lceil N/M\rceil$ 个 checkpoint、每个 $W\cdot L$ 边界 KV，相对完整历史 full-KV 口径 $\approx W/M$（$M{=}1024\Rightarrow 1/8$）。$M$ 越大存储越省、恢复越长。
...
| B 层维 checkpoint | 不省计算 | 省 cache 内存 | 中 | ❌ 不解决计算 |
| C C4/C128 周期对 + C4 skip-writeback | 省 ~54%@D=5500（与基线持平） | ✅ C128 完整 | 高 | 计算持平、cache 更优 |
| D token 维周期存储 | 省 ~81%@D=5500（$M{=}1024$，$D\ge M$） | ✅ 边界 checkpoint 常驻 | 高 | $D$ 大时最省、门槛低 |
| 松绑全局 min（§13.2） | 0 计算（协议重构） | ✅ 完整 | 中 | 依赖 skip+staircase |

### 15.8 更多探索性方向

沿 A–D 之外再列几个脑洞，给计算与判断（多数落存储/精度轴或退化为已有方向）：

- **E. 分层差异化间距 $M$**：C4/C128 层用不同 $M$（C128 写回便宜→$M_{128}$ 大、少存；C4 写回贵→$M_4$ 正常）。计算：hidden-state GEMM 与 cr 无关，总 $21M_4{+}20M_{128}{+}2M_{sw}$，分层无计算增益（同 $M\cdot L$）。判断：计算不省于 D，存储可优化。
- **F. 仅 compressor-state checkpoint**：只存 compressor state（$\le 2048\cdot 4=8$ KB/层，远小于 SWA $W\cdot 584\approx 75$ KB/层），不存 SWA ring。计算：SWA attention 恢复仍需 ring KV → 锥仍回溯，不截断。判断：单独不成立，须含 SWA（即 D）。
- **G. 层错位 checkpoint**：不同层 checkpoint 错开 token 位置，均摊单点写入峰值。计算：每层最近 checkpoint $r<M{+}L\delta$，恢复略增。判断：存储工程优化，计算略劣。
- **H. 量化/低秩 checkpoint**：checkpoint 存 int4 量化或低秩 KV。计算：反量化 cheap，恢复计算不变。判断：精度-存储权衡，计算不省。
- **I. APC 命中点 B 作天然 checkpoint**：B 是 G1–G4 命中末端，`[0,B)` 在，B 本身免费。计算：从 B 恢复 `[B,A)` = 当前 delta 段；段内 D 需 `[B,A)` 内独立 checkpoint，B 是其下界。判断：B 端免费，与 D 叠加。
- **自适应 $M=f(D)$**：D 大用大 $M$。计算：$\min(D,M)\cdot L$，自适应只选 $M$，不改变形式。判断：参数调优，非新机制。

> **小结**：计算轴上 D（及 D+staircase）是 token 维天花板；E/G/H 落存储/精度轴，F 退化，I 的 B 端免费可叠加。新计算节省仍来自"截断依赖锥"——要么截左缘（staircase）、要么截高度（D）。
...
6. **松绑 min 不省计算**：`[B,A)` 的 SWA/compressor 块哈希缓存未填充，松绑后仍须显式重建——同一依赖锥、同一计算量。计算节省来自 skip+staircase，与是否松绑无关（§13.2）；token 维周期存储（方向 D）是另一正交变体，见 §15.6。
7. **方向 C（C4/C128 层间差异化）**：计算省量 ~54%，与原全三角形持平（非 75%）；价值在 C128 层近乎免费保持 cache 完整、C4 层 skip-writeback 独立成立（§15.5）。属层维方向。
8. **方向 D（token 维周期存储）**：每 $M$ token SWA checkpoint 截断依赖锥回溯，恢复从 staircase 的 $W_{eff}\cdot L/2$（118k）降到 $M\cdot L$（$M{=}1024\to 44k$），叠加 staircase 得 $M\cdot L/2$（22k）。$D\ge M$ 即生效（门槛低于 staircase 的 $D\ge W_{eff}$），$D$ 大时最优、$D$ 小时与 staircase 互补。前提：checkpoint 独立常驻、full-state、非当前实现（§15.6）。
9. **落地顺序（依赖栈）**：skip-writeback（基础）→ staircase（省 hidden-state GEMM 大头，依赖 skip）→ 方向 D（截断回溯锥，进一步降到 $M\cdot L$）→ 松绑 min（可选，零计算节省，协议重构）。计算节省由前三者决定。更多探索性方向见 §15.8。
