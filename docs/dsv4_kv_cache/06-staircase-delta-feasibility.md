...
→ **C128 层写回成本比 C4 层低 34×**（28875 / 840）。这是方向 C（§15.5）的物理基础。注意此处的"周期"是**层间交替**，与 §15.6 的 token 维周期存储是不同概念。
...
保留 K 层完整 cache、其余 43−K 层做三角形。先看朴素选法——保留**浅层**前 K 层：分界层 $K$ 处深层三角形的底宽为 $|R_K| \approx 1 + (43-K)(W-1)$（对应 `swa-kv-offloading-analysis.md` §4 的浅层边界带）。浅层 K 层若只为喂深层三角形，算边界带 `K·|R_K|` 即可（低于全三角形）；若要保留完整浅层 cache 供复用，需算满 `K·D`（高于全三角形）。§15.3.1 证明保留**深层**反而更优。
...
> **与 DSV4 层间结构契合**：C128 层（cr=128）写回极低（840 条，§15.1）且与 C4 交替遍布浅中深，故"存深层"在 DSV4 上部分天然免费——C128 近乎免费保持完整，与"存深层更优"同向。写回主体是 C4 层（28875 条），其浅/深差异已含上表。若叠加"保留部分层 cache"，**深层保留 + C4 staircase 兜底浅层**是计算与复用兼优的组合（与方向 C 同向，§15.5）。
