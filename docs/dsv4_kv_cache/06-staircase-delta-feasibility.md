...
保留 K 层完整 cache、其余 43−K 层做三角形。先看朴素选法——保留**浅层**前 K 层：分界层 $K$ 处深层三角形的底宽为 $|R_K| \approx W+(43-K)(W-1)$（top-W 口径，与 §11.4 一致；`swa-kv-offloading-analysis.md` §4 用 top-1 口径得 $1+(L-m)(W-1)$，差一个 $W-1$）。浅层 K 层的算法分两种：

- **只算必需**（浅层 KV 已长期缓存，仅恢复边界带喂深层）：$K\cdot|R_K|$ + 深层锥。K=21 时 $\approx 61k+32k=93k$，**低于**全三角形（117k）——浅层保留仍省于不保留。
- **算满**（浅层 KV 未缓存，本步算全 delta 宽度以同时填浅层 cache 供后续复用）：$K\cdot D$ + 深层锥。K=21、D=5500 时 $\approx 116k+32k=148k$，高于全三角形但仍**远低于**矩形（236k）。

两种都不劣于矩形；算满的额外代价换的是浅层 cache 完整性（供未来命中复用）。§15.3.1 证明保留**深层**恢复计算更低。
...
- **C128 层全量保留**：cr=128，全 delta 写回也只 `D/128·20 ≈ 859` 条（D=5500），可忽略 → C128 cache 完整，后续命中可复用（原全三角形会破坏所有层完整性）。
- **C4 层是写回主体**：`D/4·21 = 28875` 条，三角形 + skip-writeback 省下。
- C4 层 skip-writeback 独立于三角形也成立——这才是 DSV4 层间结构带来、可独立落地的收益。

> **"破坏完整性"的含义**：staircase 每层只算三角形位置，左缘 delta token `[B, B+D-W_eff)` 在所有层都不算 → 其 KV 从不写回 → delta 段 `[B,A)` 留下三角形空洞（值正确但有缺位，非损坏）。当前 decode 无碍（右缘 SWA 窗口已缓存）；但后续请求若共享前缀 `[B,A)`，APC 按 block hash 查会因左缘 block 缺失而命中变短 → 那段须重算。即 staircase 用"放弃 delta 前缀的未来复用"换当前省计算。方向 C 对 C128 层全量写回（所有 D token 都写）→ C128 block 完整可命中，只有 C4 层留洞。
