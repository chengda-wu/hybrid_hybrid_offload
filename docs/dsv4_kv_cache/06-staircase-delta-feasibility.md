...
方向 A 默认存浅层（前 K 层）。但若目标是在 staircase 之外保留部分层 cache 供后续命中复用，**存深层（后 K 层）恢复计算显著更低**。原因是 SWA 依赖锥向低层、向更早 token 扩张，两种存法挡的是锥的不同端：

- **存深层**（浅层 1…m 缺失）：锥只在浅层 m 层扩张，到达分界层后被深层已缓存 KV 截断，后续是沿当前位置的窄竖线（每层 W 个位置）。形状 = 小锥 + 窄竖线。
- **存浅层**（深层 m+1…L 缺失）：深层缺失段仍形成锥，其底（分界层）很宽 $|R_m|=W+(L-m)(W-1)$；浅层为喂这个宽底，须把 $|R_m|$ 个位置逐层推过 m 层 = `m·|R_m|` 的矩形带。形状 = 宽矩形带 + 高层锥。

对齐本文口径（顶层需 $W=128$、main MLA 已缓存），保存相同层数时恢复计算对比（$L=43,W=128$）：
...
存一半（21 层）时，深层恢复计算仅为浅层的 **37%**。`swa-kv-offloading-analysis.md` §4/§5 在"从零恢复单 token"口径下得到同样的浅/深不对称结论——深层保存护住的是"靠近输出侧、当前 step 真正消费"的窗口状态，恢复路径退化为竖线；浅层保存护住离输出远的层，仍要把一批位置推过未保存的高层，矩形带成为大头。

> **与 DSV4 周期结构的契合**：DSV4 的 C128 层（cr=128）写回成本极低（840 条，§15.1），而 C128 与 C4 严格交替、遍布浅中深。这意味着"存深层"在 DSV4 上部分天然免费——C128 层近乎免费地保持完整，与"存深层更优"同向。真正要省的写回主体是 C4 层（28875 条），其恢复计算的浅/深差异已含在上表。故若叠加"保留部分层 cache"，**深层保留 + C4 staircase 兜底浅层**是计算与复用兼优的组合（与方向 C 同向，§15.5）。
...
1. DSV4 的 C4/C128 周期结构使 C128 层写回成本比 C4 低 34×（§15.1）——原 staircase 没利用的差异。
2. 层间连续性（§15.2）禁止逐层独立三角化，只能按 (C4, C128) 周期对整体处理。
3. 方向 C 计算省量 ~54%，与原全三角形（~50%）持平（$W_{eff}\approx 5208$ 非减半），**非 75%**。
4. 方向 C 的真正价值在写回/cache 复用：C128 层近乎免费保持完整，C4 层 skip-writeback 独立于三角形也成立。
5. 松绑 min 不省计算、依赖 skip+staircase（§13.2）——计算节省与是否松绑无关。
