...
2. **SWA 窗口内的历史 attention 状态**，即 $\bigl\{KV_t^{(\ell)}\bigr\}_{t \in \mathcal{W}_N}$
...
3. 读取第 $\ell$ 层窗口内缓存的 full K/V：$\bigl\{KV_t^{(\ell)}\bigr\}_{t \in \mathcal{W}_N}$
...
$$
\bigl\{KV_t^{(\ell)}\bigr\}_{t \in \mathcal{W}_N}
$$
...
2. 将这些位置从输入侧推到第 $m$ 层，得到 $\bigl\{h_t^{(m)}\bigr\}_{t\in R_m}$
...
$$
\mathcal{B}_c = \bigl\{ KV_t^{(\ell)} \mid \ell=1,\dots,L, t \in \{c-W+1,\dots,c\} \bigr\}
$$
