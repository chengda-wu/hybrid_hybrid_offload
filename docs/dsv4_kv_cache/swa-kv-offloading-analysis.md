...
## 9. 两段 Gap 的 Pareto 计算公式

本节记录用于生成 Pareto 曲线的简化几何模型。网络前 $l$ 层使用 token checkpoint gap $g$，后 $L-l$ 层使用 gap $2g$。计算量按 hidden-state token-layer 单元计，SWA 依赖每向浅层经过一层向前扩张
...
且至少一个不等式严格成立，则 $(l,g)$ 是 Pareto 最优点。上述可行域共生成 58695 个采样点，其中 3164 个为 Pareto 点。数据和绘图实现见 `dsv4_checkpoint_points.csv` 与 `plot_checkpoint_pareto.py`。
