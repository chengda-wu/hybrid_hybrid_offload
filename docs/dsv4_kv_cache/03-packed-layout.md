...
物理块内按 page_size 分 3 个 region，每 region 内按 slot 切片。每个 slot 容纳跨组同 `(ps, slot_idx)` 的所有层：

```
偏移       ps      slot#  region 内含层(跨组)                      字节
─────────────────────────────────────────────────────────────────────
0          37,440  22     C4mla(21) + C4comp_sw(21)               823,680
                          + C128comp_sw(20) + SWA_sw(G1,22)
                          + SWA_sw(G2,21)   ← comp pad 到 37440
                          (slot 0-19: 5层 / slot 20: 4层
                           / slot 21: 1层, 详见下方)
─────────────────────────────────────────────────────────────────────
...
**37,440 区各 slot 的层组成**（层数随 slot 递增而递减，因为不同组层数不同）：

| slot | 层数 | 内含层 |
|------|------|--------|
| 0–19 | 5 | `C4mla` + `C4comp_sw` + `C128comp_sw` + `SWA_sw`(G1) + `SWA_sw`(G2) |
| 20 | 4 | `C4mla` + `C4comp_sw` + `SWA_sw`(G1) + `SWA_sw`(G2)（无 `C128comp_sw`，C128 只 20 层） |
| 21 | 1 | `SWA_sw`(G1)（G1 第 22 层；G2 只 21 层到此为止） |
...
**为什么仍值得**：packed 布局用"单 block 部分填充"换"统一 pool 零碎片"。若每个 group 独立 pool，用量小的 group pool 闲置、用量大的 group 不够用——内部碎片。共用 pool 让总利用率 = 总需求 / 总容量，无内部碎片。对 DSV4 这种 7 类异构 cache 的模型，统一 pool 的简化（无需为每类 cache 单独规划容量）被认为值得单 block 的填充率损失。
