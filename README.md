...
#### SGLang — 24.36 GB（spec 默认 K=2, ring 16/256, 43/44 scaled）／ 15.58 GB（nonspec, ring 8/128）
...
SGLang（spec 默认 K=2）24.36 GB vs vLLM 9.60 GB，差值 +14.76 GB（总盘相减）。主要来源：C128 compressor state spec ring(256) 远大于 vLLM packed。nonspec 下为 15.58 GB（差值 +5.98 GB）。vLLM packed 布局下各组容量不可简单加减（共享 bucket），差值以总盘为准。
...
- **FP4 indexer 未建模**：c4_indexer 按 132 B/token 计算（非 fp4 模式）。若启用 `enable_deepseek_v4_fp4_indexer`（→68 B/token），sim 高估 indexer KV ~2×（绝对值 ~35 MB，占比 <0.25%）
