...
| C4 Compressor ring | `swa_slots × ring(8) × 8192B × 21L` | 21L | 1.05 GB |
...
SGLang 15.56 GB vs vLLM 9.60 GB，差值 +5.96 GB（总盘相减）。主要正贡献：C128 compressor state（SGLang float32 ring 远大于 vLLM packed）；主要负贡献：SWA ring 10% 密度 vs vLLM 满密度。vLLM packed 布局下各组容量不可简单加减（共享 bucket），差值以总盘为准。
