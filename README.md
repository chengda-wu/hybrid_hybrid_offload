...
| `--acceptance-rates` | — | 逐位置**边际**接受率（真实 speculator 上实测的 `P(draft_i 被接受)`，口径A：分母=所有 spec-decode step），空格分隔 **K 个浮点数**，须**非递增**。如 `0.9 0.7 0.5 0.3`。该值已含 draft 正确性 + target 验证，无需另指定 draft 质量。仿真器内部按链式断裂语义转成条件率 `cond[i]=rate[i]/rate[i-1]` 采样，使实测边际率复现输入；报表 `per_position_acceptance_rates` 可核对（长序列收敛于输入；短序列因 EOS 附近 draft 越界，后段位置会偏低，属口径A的固有表现）。仅 `--accept-mode per_position` 时生效，**长度必须 ≥ K**；未提供时回退到 0.5 条件率并打印 warning |
