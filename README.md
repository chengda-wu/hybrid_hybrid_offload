...
SGLang 15.56 GB vs vLLM 9.60 GB，差值 +5.96 GB。主要贡献：C128 compressor state 池（SGLang float32 ring buffer vs vLLM packed pool，+5.49 GB）、SWA 满密度差异（−4.13 GB）、C4 compressor state（−1.77 GB），其余项相互抵消。vLLM 侧以 packed 共享池折算。
