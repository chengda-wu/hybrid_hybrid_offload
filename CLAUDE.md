...
- **Don't hardcode model parameters.** DeepSeek V4 defaults come from the actual HF `config.json` (verified against `deepseek-ai/DeepSeek-V4-Flash`). Any new model should be configurable via `--model-config-path`.
